...
public class TextureManager
{
	...
	{
		//Before load new texture dispose old ones
		if(texture!=null)
		{
			Object[] list = texture.keySet().toArray();
			int i=0;
			while(i<list.length)
			{
				texture.get(list[i]).dispose();
				i++;
			}
		}
		...
		{
			texture= new HashMap<String,Texture>();
			texture.put("null",new Texture(new Pixmap(1,1,Pixmap.Format.RGB888)));
		}
	}
